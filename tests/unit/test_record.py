...
def test_create_stores_record_id_internally(mocker, env, input_record_id):
    mocker.patch(
        'certbot_dns_auth.do_domain.record.Resource.create')
    mocker.patch(
        'certbot_dns_auth.do_domain.record.Resource.__int__',
...
def test_create_properly_calls_http_create(mocker, env):
    stub_http_create = mocker.patch(
        'certbot_dns_auth.do_domain.record.Resource')
...
def test_printer_calls_printer(mocker, env):
    stub_printer = mocker.patch(
        'certbot_dns_auth.do_domain.record.stdout')
    record_id = 918342
