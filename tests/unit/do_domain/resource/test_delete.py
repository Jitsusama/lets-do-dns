...
from lets_do_dns.errors import RecordDeletionFailure
...
def test_raises_authentication_failure_on_requests_exception(
    ...
    with pytest.raises(RecordDeletionFailure):
        resource.delete()
...
def test_passes_handled_exception_to_authentication_failure(
    ...
    mock_record_creation_failure = mocker.patch(
        'lets_do_dns.do_domain.resource.RecordDeletionFailure',
        return_value=RecordDeletionFailure)
